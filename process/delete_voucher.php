...
/**
 * Delete Voucher Data
 * Hapus data voucher beserta semua foto dan PDF
 */
...
try {
    ...
    if ($kode_kantor !== '000' && $voucher['kode_kantor'] !== $kode_kantor) {
        echo json_encode(['success' => false, 'message' => 'Anda tidak punya akses untuk menghapus data ini']);
        exit;
    }
    
    // Get all photos
    $stmt = $conn->prepare("SELECT foto_path FROM voucher_foto WHERE voucher_data_id = ?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $result = $stmt->get_result();
    $photos = [];
    ...
    if (!empty($voucher['pdf_path'])) {
        $pdf_full_path = __DIR__ . '/../' . $voucher['pdf_path'];
    ...
    $stmt = $conn->prepare("DELETE FROM voucher_foto WHERE voucher_data_id = ?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $stmt->close();
    
    // Delete voucher data
    $stmt = $conn->prepare("DELETE FROM voucher_data WHERE id = ?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $stmt->close();
